...
import { updateSliderVerisonAPI } from "../../../../api/api";
...
const Update = ({listId}) => {
  ...
  const handleUpdateChangeStatus = ({ meta, file }, status) => {
    setImgFile(file);
  };
  const handleUpdate = async (e) => {
    ...
    await updateSliderVerisonAPI(listId, formData);
  ...
  const populateData = () => {
    if (ifExists) {
      setTitle(ifExists?.dataObj.sliderversionslider_title);
      setTitleFontSize(ifExists?.dataObj.sliderversionslider_titlefontsize);
      setText(ifExists?.dataObj.sliderversionslider_text);
      setTextFontSize(ifExists?.dataObj.sliderversionslider_textfontsize);
      setButton(ifExists?.dataObj.sliderversionslider_buttontext);
      setButtonFontSize(ifExists?.dataObj.sliderversionslider_buttonfontsize);
      setButtonUrl(ifExists?.dataObj.sliderversionslider_buttonURL);
      setSerialNumber(ifExists?.dataObj.sliderversionslider_serialnumber);
      setImgFile(ifExists?.dataObj.sliderversionslider_image);
    }
  };
        ...
        <Row>
          <Col>
            <FormGroup>
              <Label>{"Image "}</Label>
              <Dropzone
                onChangeStatus={handleUpdateChangeStatus}
                maxFiles={1}
                multiple={false}
                canCancel={false}
