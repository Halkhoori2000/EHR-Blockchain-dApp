...
const SideBarLayout = (props) => {
  ...
  useEffect(() => {
    ...
    const userType = profile[3]
    console.log("userType", userType)
    const updateMenu = MENUITEMS.map(items => {
      items.Items = items.Items.filter((item) => {
        if (userType == "1") {
          if (item.title == "Profile" || item.title == "Doctors List" || item.title == "Prescriptions List" || item.title == "Reports List") {
            return true;
          }
        }
        else if (userType == "2") {
          if (item.title == "Profile" ||item.title == "Patients List" || item.title == "Reports List") {
            return true;
          }
        }
        else if (userType == "3") {
          if (item.title == "Profile" || item.title == "Prescriptions List" || item.title == "Reports List") {
            return true;
          }
        }
        return false;
    ...
    console.log("UPDATE MENUITEMS", updateMenu)
